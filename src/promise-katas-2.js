...
const dog = () => {
   return fetch('dogs')
            .then(result => {
                  ...
                  const naughtiestDog = dogs.find(dog => {
                     let score = 0;
                     if (dog.naughty > score) { 
                       score = dog.naughty; 
                    }
                    return score;
                  })
...
const joke = () => {
   const getQuestion = fetch('jokes', 'question')
   const getAnswer = fetch('jokes')
   return Promise.all([getQuestion, getAnswer]).then((values) =>{
      const question = values[0]
      const answer = values[1]
      return { question: question.joke, answer: answer.answer }
   });
};
